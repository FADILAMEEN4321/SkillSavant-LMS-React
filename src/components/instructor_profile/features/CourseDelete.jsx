...
import { deleteCourse } from "./../../../api/instructorAPI";
...
const CourseDelete = ({
  ...
  const handleCourseDelete = async (courseId) => {
    ...
    try {
      ...
      const updatedCourseList = originalCourses.filter(
        (course) => course.id !== courseId
      );
      setOriginalCourses(updatedCourseList);
  ...
  return (
    <>
      <div
        onClick={() =>
          document.getElementById(`delete_modal_${course.id}`).showModal()
        }
      ...
      <dialog id={`delete_modal_${course.id}`} className="modal">
        <div className="modal-box">
          ...
          <p className="text-sm mb-4 text-center font-bold">
            Students already enrolled in this course will be able to access
            course.
          </p>
